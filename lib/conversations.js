...
export async function listConversationsForClients(clientCodes = []) {
  const scopedClientCodes = [...new Set(clientCodes.map(clean).filter(Boolean))];
  if (scopedClientCodes.length === 0) return [];
  ...
  const result = await query(
    ...
    from conversations c
    left join messages m on m.conversation_id = c.id
    where c.client_code = any($1::text[])
    group by c.id
    order by c.updated_at desc`,
    [scopedClientCodes]
  );
